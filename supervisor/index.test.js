  ...
  const proc = spawn(process.execPath, [ENTRY], {
    env: {
      ...
      /* Nothing listens on port 1. Refused instantly and forever, so the
         "njsPC is unreachable" state is reached deterministically. */
      NJSPC_URL: "http://127.0.0.1:1",
...
describe("a supervisor that cannot reach njsPC", () => {
  ...
  describe("refusals, which are answers and not silence", () => {
    ...
    it("refuses a program that has no circuit yet, by name", async () => {
      const ack = await client.intent("startProgram", { id: "filtration" });
      expect(ack.ok).toBe(false);
      expect(ack.error).toMatch(/Filtration/);
      expect(ack.error).toMatch(/commissioning/);
    });
