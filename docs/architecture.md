...
| **Mains restored, but the Pi cannot be reached** | **nothing** | **Not designed for, and it happened.** September 2026: an outlet pull with no clean halt, after which `poolctl.local` would not resolve and SSH by name failed, for days, with the Pi showing power and SD activity. A second power cycle brought it back. **What failed is unknown and will stay unknown** — the journal is volatile, so that boot left no record — and it is not even known whether the Pi was off the network or on it with only its name gone: the sweep used to look for it filtered by a Raspberry Pi MAC prefix list that did not include this Pi's own. The row above says the relays are *"corrected at the next boot"*, which assumes the next boot succeeds and can be seen to. See PRD §7, *Storage note* |
